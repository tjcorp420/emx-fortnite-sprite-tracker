...
import { claimAnonymousAccount, createInvite, createTracker, deleteAccount, deleteTracker, ensureDefaultTracker, getLeaderboard, getProfile, getSession, isSupabaseConfigured, listTrackers, loadProgress, redeemInvite, refreshCloudSession, saveAchievements, saveProgress, setLeaderboardTracker, signInAnonymously, signInWithPassword, signOut, signUpWithPassword, supabase, updateLeaderboardOptIn, updateProfileCosmetics, updateTracker, type CloudTracker, type LeaderboardRow } from '../lib/cloud';
...
export default function Home() {
  ...
  const update = (id: string, patch: Progress[string]) => {
    ...
    if (patch.owned !== undefined && patch.owned !== current.owned) showActivity(patch.owned ? 'Sprite indexed' : 'Removed from collection', `${sprite?.name || 'Sprite'} ${patch.owned ? 'is now owned' : 'is no longer marked owned'}`, 'collect');
    else if (patch.mastered !== undefined && patch.mastered !== current.mastered) showActivity(patch.mastered ? 'Mastery recorded' : 'Mastery removed', `${sprite?.name || 'Sprite'} ${patch.mastered ? 'is mastered' : 'is no longer mastered'}`, 'master');
    else if (patch.favorite !== undefined && patch.favorite !== current.favorite) showActivity(patch.favorite ? 'Favorite saved' : 'Favorite removed', sprite?.name || 'Sprite', 'favorite');
