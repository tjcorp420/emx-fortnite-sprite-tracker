...
export default function Home() {
  ...
  const tabSprites = catalogTab === 'current' ? currentSeasonSprites : legacySprites;
  const values = (key: keyof Sprite) => Array.from(new Set(tabSprites.map((sprite) => String(sprite[key])))).sort();
  const filtered = useMemo(() => catalog.filter((s) => {
    ...
    const matchesRelease = catalogTab === 'current' || release === 'all' || (release === 'released' ? s.released : !s.released);
    return inSelectedTab && matchesRelease && (!query || `${s.name} ${s.id}`.toLowerCase().includes(query.toLowerCase())) && (type === 'all' || s.type === type) && (variant === 'all' || s.variant === variant) && (rarity === 'all' || s.rarity === rarity) && (status === 'all' || (status === 'owned' ? p.owned : status === 'mastered' ? p.mastered : status === 'needs-mastering' ? p.owned && !p.mastered : status === 'favorites' ? p.favorite : !p.owned));
