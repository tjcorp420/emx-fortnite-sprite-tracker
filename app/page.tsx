...
const GITHUB_RELEASES_API = 'https://api.github.com/repos/tjcorp420/emx-fortnite-sprite-tracker/releases/latest';
function UpdateButton() {
  ...
  const check = async () => {
    ...
    try {
      const response = await fetch(`${GITHUB_RELEASES_API}?cacheBust=${Date.now()}`, { cache: 'no-store', headers: { Accept: 'application/vnd.github+json' } });
      if (!response.ok) throw new Error('GitHub Releases is unavailable right now.');
      ...
      if (!asset) throw new Error('The latest release does not include the EMX Windows installer.');
...
function AuthModal({ user, leaderboardOptIn, setLeaderboardOptIn, close, continueAnonymous, onSignedIn, onSignedOut }: { user: AuthUser | null; leaderboardOptIn: boolean; setLeaderboardOptIn: (enabled: boolean) => Promise<void>; close: () => void; continueAnonymous: () => Promise<void>; onSignedIn: (user: AuthUser) => void; onSignedOut: () => Promise<void> }) {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setBusy(true); setError(''); try { const result = mode === 'claim' ? await claimAnonymousAccount(email, password, displayName || 'EMX Trainer') : mode === 'create' ? await signUpWithPassword(email, password, displayName || 'EMX Trainer') : await signInWithPassword(email, password); if (result.error) throw result.error; if (result.data.user) onSignedIn({ id: result.data.user.id, email: result.data.user.email, is_anonymous: result.data.user.is_anonymous }); else setMessage('Check your email to finish creating the account.'); } catch (err: any) { setError(err.message || 'Authentication failed.'); } finally { setBusy(false); } };
