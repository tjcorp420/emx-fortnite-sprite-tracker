...
// This is the stable Vercel project alias, intentionally separate from the
// installed PWA origin. iOS can otherwise relaunch the PWA at its start URL
// and discard the one-time fragment Supabase adds to the reset link.
export async function requestPasswordReset(email: string) { if (!supabase) throw new Error('Supabase is not configured.'); const redirectTo = 'https://sprite-tracker-to-track-all-sprites-tjcorp420s-projects.vercel.app/?recovery=1'; return supabase.auth.resetPasswordForEmail(email, { redirectTo }); }
