...
export async function requestPasswordReset(email: string) { if (!supabase) throw new Error('Supabase is not configured.'); const redirectTo = 'https://sprite-tracker-to-track-all-sprites.vercel.app/?recovery=1'; return supabase.auth.resetPasswordForEmail(email, { redirectTo }); }
